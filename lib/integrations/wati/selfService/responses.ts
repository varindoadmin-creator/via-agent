...
export function askWhichCustomerForInquiry(companyNames: string[]): string {
  ...
  return `Baik Kak, informasi ini ingin dicek untuk:\n\n${options}`;
}

export function askWhichOpenOrder(soNumbers: string[]): string {
  return `Baik Kak, saat ini ada beberapa pesanan aktif. Pesanan yang dimaksud ${soNumbers.join(' atau ')}?`;
}
...
export function orderStatusReply(order: CustomerSafeOrderStatus): string {
  return `Baik Kak, pesanan ${order.orderNumber} saat ini ${ORDER_STATUS_PHRASE[order.status]}.`;
}

export function orderNotFound(soNumber: string): string {
  return `Mohon maaf Kak, kami belum menemukan pesanan ${soNumber} pada akun perusahaan Kakak. Boleh dicek kembali nomornya?`;
}

export function needOrderReference(): string {
  return 'Baik Kak, boleh diinformasikan nomor SO/pesanannya?';
}

export function orderHistoryReply(orders: CustomerSafeOrderStatus[]): string {
  if (orders.length === 0) return 'Baik Kak, belum ada riwayat pesanan yang tercatat untuk akun ini.';
  const lines = orders.map(o => `${o.orderNumber} — ${o.orderDate}`).join('\n');
  return `Baik Kak, berikut ${orders.length} pesanan terakhir:\n\n${lines}\n\nApakah Kakak ingin saya cek periode tertentu?`;
}

export function lastOrderReply(order: CustomerSafeOrderStatus | null): string {
  if (!order) return 'Baik Kak, belum ada pesanan yang tercatat untuk akun ini.';
...
export function invoiceStatusReply(invoice: CustomerSafeInvoice): string {
  ...
  if (invoice.status === 'UNPAID' || invoice.status === 'OVERDUE' || invoice.status === 'PARTIALLY_PAID') {
    return `Baik Kak, ${base}, dengan saldo ${formatIDR(invoice.balanceDue)}.`;
  }
  return `Baik Kak, ${base}.`;
}

export function invoiceNotFound(invoiceNumber: string): string {
  return `Mohon maaf Kak, kami belum menemukan invoice ${invoiceNumber} pada akun perusahaan Kakak. Boleh dicek kembali nomor invoicenya?`;
}

export function needInvoiceReference(): string {
  return 'Baik Kak, boleh diinformasikan nomor invoicenya?';
}

export function invoiceDocumentSent(invoiceNumber: string): string {
  return `Baik Kak, berikut invoice ${invoiceNumber}.`;
}

export function invoiceDocumentSendFailed(): string {
  return 'Mohon maaf Kak, saat ini kami belum dapat mengirimkan dokumen tersebut. Kami bantu teruskan untuk pengecekan.';
}

export function outstandingInvoicesReply(customerLabel: string, invoices: CustomerSafeInvoice[]): string {
  if (invoices.length === 0) return `Baik Kak, saat ini tidak ada invoice yang belum lunas untuk ${customerLabel}.`;
  const lines = invoices.map(i => `${i.invoiceNumber} — ${formatIDR(i.balanceDue)}${i.dueDate ? ` — jatuh tempo ${i.dueDate}` : ''}`).join('\n');
  return `Kak, untuk ${customerLabel} terdapat ${invoices.length} invoice yang masih memiliki saldo:\n\n${lines}`;
}
...
export function paymentStatusReply(outcome: PaymentStatusOutcome): string {
  ...
  if (outcome.outcome === 'RECORDED') return `Baik Kak, pembayaran untuk ${outcome.invoice.invoiceNumber} sudah tercatat.`;
  if (outcome.outcome === 'PARTIALLY_RECORDED') return `Baik Kak, ${outcome.invoice.invoiceNumber} sudah sebagian terbayar, dengan sisa saldo ${formatIDR(outcome.invoice.balanceDue)}.`;
  // Brief section 21: never accuse the customer, never auto-mark paid — route for finance follow-up.
  return `Baik Kak, pembayaran tersebut belum terlihat sebagai pembayaran teralokasi di sistem kami untuk ${outcome.invoice.invoiceNumber}. Kami bantu teruskan untuk pengecekan.`;
}

export function receivableSummaryReply(customerLabel: string, summary: CustomerReceivableSummary): string {
  if (summary.invoiceCount === 0) return `Baik Kak, saat ini tidak ada saldo invoice terbuka untuk ${customerLabel}.`;
...
export function deliveryStatusReply(orderNumber: string, status: string): string {
  return `Baik Kak, pesanan ${orderNumber} saat ini ${DELIVERY_STATUS_PHRASE[status] ?? 'sedang kami periksa'}.`;
}
...
export function upstreamUnavailable(): string {
  return 'Maaf Kak, saat ini status tersebut belum dapat kami cek. Kami bantu teruskan untuk pengecekan.';
}
