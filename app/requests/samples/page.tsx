...
const STATUS_OPTIONS = ['New', 'Requested to Vendor', 'Sent to Customer', 'Cancelled'];
...
function SampleRow({ req, onStatusChange, savingId, selected, onToggleSelect }: {
  req: SampleRequest;
...
export default function SampleRequestsPage() {
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete ${selectedIds.size} sample request${selectedIds.size === 1 ? '' : 's'}? This cannot be undone.`)) return;
    setDeleting(true);
    try {
      const res = await fetch('/api/requests/samples', {
        method: 'DELETE',
