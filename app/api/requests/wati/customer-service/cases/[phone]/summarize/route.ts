...
import { summarizeConversation } from '@/lib/customerService/copilot';
import { supabaseSelect } from '@/lib/supabase/rest';
import { isJarvisAdminCopilotEnabled } from '@/lib/customerIdentity/featureFlags';
...
// POST /api/requests/wati/customer-service/cases/[phone]/summarize — brief
// section 26: internal-only, never sent to the customer.
export async function POST(req: NextRequest, { params }: { params: Promise<{ phone: string }> }) {
  ...
  if (!isJarvisAdminCopilotEnabled()) return NextResponse.json({ success: false, error: 'Jarvis Admin copilot is disabled (JARVIS_ADMIN_COPILOT_ENABLED is not set to true).' }, { status: 503 });
  ...
  try {
    ...
    return NextResponse.json({ success: true, summary, context });
  } catch (error) {
    console.error('[CustomerServiceSummarize]', error instanceof Error ? error.message : 'unknown error');
    return NextResponse.json({ success: false, error: 'Unable to generate summary.' }, { status: 500 });
  }
}
