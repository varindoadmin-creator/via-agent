...
import { returnToAuto } from '@/lib/customerService/caseActions';
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ phone: string }> }) {
  ...
  try {
    const result = await returnToAuto(decodeURIComponent(phone), role);
    return NextResponse.json({ success: true, case: result });
  } catch (error) {
    return NextResponse.json({ success: false, error: error instanceof Error ? error.message : 'Return to VIA failed.' }, { status: 409 });
  }
}
